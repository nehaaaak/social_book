...
def index(request):
    ...
    if not feed_list:
        ...
        for user in unique_users:
            first_post= Post.objects.filter(user=user).first()
            if first_post:
                feed_list.append(first_post)

    # fetch comments for each post
    comments_dict = {post.id: Comment.objects.filter(post=post) for post in feed_list}
    ...
    return render(request, 'index.html', {'user_profile':user_profile, 'posts':feed_list, 'comments_dict': comments_dict, 'suggestions_username_profile_list':suggestions_username_profile_list[:4]})
